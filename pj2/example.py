...
class BiLSTM_CRF(nn.Module):
    def __init__(self, dataset, embedding_dim, hidden_dim, device='cpu'):
        ...
        self.w_dic_size = len(dataset.w_dic)  # 词表大小
        self.tagset_size = len(dataset.t_dic)  # 标签个数
        self.device = device
        # 记录状态，'train'、'eval'、'pred'对应三种不同的操作
        self.state = 'train'  # 'train'、'eval'、'pred'

        self.word_embeds = nn.Embedding(self.w_dic_size, embedding_dim)
...
class CRF:
    def __init__(self, dataset, device='cpu'):
        ...
        self.tagset_size = len(self.t_dic)
        self.device = device
        ...
        self.STOP_TAG = "END"
        self.transitions.data[self.t_dic[self.START_TAG], :] = -10000
        self.transitions.data[:, self.t_dic[self.STOP_TAG]] = -10000

    def _forward_alg(self, feats, seq_len):
        # 手动设置初始得分，让开始标志到其他标签的得分最高
        init_alphas = torch.full((self.tagset_size,), -10000.)
        init_alphas[self.t_dic[self.START_TAG]] = 0.
        ...
        # 手动干预,加上结束标志位的转移概率
        terminal_var = forward_var + self.transitions[self.t_dic[self.STOP_TAG]].unsqueeze(0).repeat(feats.shape[0], 1)
        # 得到最终所有路径的分数和
    ...
    def _score_sentence(self, feats, tags, seq_len):
        ...
        start = torch.tensor([self.t_dic[self.START_TAG]], device=self.device).unsqueeze(0).repeat(feats.shape[0], 1)
        ...
        for batch_i in range(feats.shape[0]):
            ...
            # 最后加上结束标志位的转移概率
            score[batch_i] += self.transitions[self.t_dic[self.STOP_TAG], tags[batch_i][seq_len[batch_i]]]
        return score
    ...
    def _viterbi_decode(self, feats):
        ...
        init_vvars[0][self.t_dic[self.START_TAG]] = 0
        ...
        # 手动加入转移到结束标签的概率
        terminal_var = forward_var + self.transitions[self.t_dic[self.STOP_TAG]]
        # 在最终位置得到最高分数所对应的索引
        ...
        # 弹出开始标签
        start = best_path.pop()
        assert start == self.t_dic[self.START_TAG]  # Sanity check
        # 将路径反转
        best_path.reverse()
        str_best_path = []
...
w_dic, t_dic, train_data, index2tag, index2word = load_data('./English/train.txt')

valid_data = load_data('./English/validation.txt')[2]
...
train_dataloader = DataLoader(train_dataset, batch_size=256, pin_memory=True, shuffle=True,
                              collate_fn=train_dataset.collate_fn)
valid_dataloader = DataLoader(valid_dataset, batch_size=256, pin_memory=False, shuffle=False,
            ...
            for sentences, tags, sen_len in valid_dataloader:
                ...
                for sentence, predict in zip(sentences, predicts):
                    for word, tag in zip(sentence, predict):
                        f.write(model.crf.index2word[int(word)]+' ')
                        f.write(tag+'\n')
                    f.write('\n')
